        ...
        ( match e with
          ...
          | Etr(n,_,_,(r0,p0)) ->
            ...
            let e0 =
              ...
              (emt_set_ptn 0 "" s "r12" emt_reg_x86 i0)^
              "\tpop rbx\n"^
              ea^
              "\tmov rdi,rbx\n"^
              "\tcall dec_r\n"^
...
and emt_get_crt_ptn s _ idx r =
  ...
  ( match r with
    ...
    | R rs ->
      ...
      (m,e2)
    | _ -> ((-1),"; emt_get_crt_ptn\n") )
and emt_set_ptn d bt s tbv idx r =
  pnt true "enter emt_set_ptn \n";
  let c0 = "emt_set_ptn "^(emt_ptn r) in
  pnt true c0;
  let open Rcd_Ptn in
  let src = "QWORD [set_ptn_vct+8*1+16*"^(string_of_int d)^"]" in
  ( match r with
    | A (-2) ->
      pnt true "dbg -2\n";
      (cmt c0)
      ...
      let (e0,_) =
        Array.fold_left
          (fun (e0,i) r ->
             ...
             in
             let e1 = emt_set_ptn (d+1) "" s tbv idx r in
             (e0^ei^e1,i+1))
          ("",0) rs in
      bt^(cmt c0)^e0
    | _ -> ";emt_set_ptn\n" )
...
and emt_ptn_set_ptn s sf tbv idx0 idx1 r0 r1 =
  ...
  ( match r0,r1 with
    | A i,_ ->
      ...
      "\tmov rbx,"^(idx0 i)^"\n"^
      "\tmov QWORD [set_ptn_vct+8*1],rbx\n"^
      (emt_set_ptn 0 bt sf tbv idx1 r1)
    | R _,A (-2) ->
      (cmt c0)
    | R _,A i ->
      let e0 =
                    ...
                    ( match pa with
                      | Eq_Agl_N(n,r) ->
                        ...
                        let es =
                          "\tmov rbx,"^r1^"\n"^
                          "\tmov QWORD [set_ptn_vct+8*1],rbx\n"^
                          (emt_set_ptn 0 bt s "r12" emt_reg_x86 rt) in
            ...
            ( match y with
              | A v ->
                ...
                let e_e =
                  ...
                  (emt_set_ptn 0 "" s_e "r12" emt_reg_x86 (A iy))^
            ...
            ( match ep,af with
              | a,_ when a=Ast.Axm._add ->
                ( try
                    ...
                    "\tpop r14\n"^
                    "\tmov QWORD [set_ptn_vct+8*1],rbx\n"^
                    (emt_set_ptn 0 "" s "r12" emt_reg_x86 iy)^
                    "\tpop rbx\n"
