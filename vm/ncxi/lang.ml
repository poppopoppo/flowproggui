...
module R = struct
  ...
  and print_atm a =
    ( match a with
      ...
      | Agl(ia,i) -> (string_of_int ia)^"◂"^(print i)
...
module Ast = struct
  ...
  and pt = ir_code ref
  and etr = ptn * pt
  and mtc_ptn = ptn * (r_n_v * eq) list
  and ptn_eq = ptn
  and eq =
    | Eq_Agl of int * ptn_eq
    | Eq_Agl_N of axm0 * ptn_eq
    | P_Cst of cst
  and cst =
    | P_N of axm0
    | P_Z of int
...
*)
  let print_test e =
    ( match e with
      | Eq_Agl(i,r) -> "°"^(string_of_int i)^"◂"^(pnt_ptn r)
      ...
      let v =
        ( try List.assoc p gns.ns_r_t
          with _ ->
            ...
            v ) in
      inst l (Var v))
let rec inst_mtc_ptn gns ns l (r,es) =
  ...
  let rec lp r0 e =
    ( match e with
      ...
      | App(Name n,e2) ->
        ...
        let n1 = IR_Glb_Call(n,r2,RP.A v) in
        (l2@[n1],A v)
      | App (e1,e2) ->
        let v0 = mk_v_r () in
        let v1 = mk_v_r () in
        ...
        let (l1,r1) = lp (A v0) e1 in
        let (l2,r2) = lp (A v1) e2 in
        ( match r1 with
          | A r1 ->
            let n1 = IR_Glb_Call(ref(Stt_Axm Axm._app),RP.R[|RP.A r1;r2|],A v2) in
            ([n0]@l1@l2@[n1],A v2)
          | _ -> err "ir_of_exp 5" )
      | ExpCst a ->
        let v = mk_v_r () in
        let p1 = IR_Exp(ExpCst a,r0,A v) in
        ([p1],A v)
      | Name n ->
        let v = mk_v_r () in
        let p1 = IR_Exp(Name n,r0,A v) in
        ([p1],A v)
      | Rcd es ->
...
and crt_rv r rv =
  ( match !r with
    | R_N n ->
      let p =
        (if n="_" then R_WC (sgn ()) else R_A (sgn ())) in
      let r0 = R_Ax p in
      Hashtbl.add rv n r0;
      r := r0;
      r
...
and rset_ptn_atm (s:RSet.t) i =
  ( match i with
    | R.Idx i when i=(-1) -> ()
    | R.Idx i -> s.(i)<-true
    | R.Agl(ra,r) -> s.(ra)<-true; let _ = (rset_ptn s r) in ()
                ...
                let (_,es,pp0) =
                  List.fold_left
                    ( fun (i,s,pp0) (t,n) ->
                        ...
                        gns.ns_e <- (epi,ref(Ctr i))::gns.ns_e;
                ...
                ( fun gv g ->
                    ( match g with
                      | Grm.Cnc (n,rs) ->
                        ...
                        gns.ns_e <- (epv,ref(Ast.Etr_V(RP.R[|RP.A(R.Idx 0);RP.A(R.Idx 1)|],RP.R[|RP.A(R.Idx 0);RP.A(R.Idx 1);RP.A(R.Agl(2,RP.A(R.Idx 3)))|])))::gns.ns_e;
                      ...
                      let (_,rts) =
                        List.fold_left
                          (fun (i,rts) (nc,_,r) ->
                             ...
                             gns.ns_e <-  (epv_i,ref(Ctr i))::gns.ns_e;
                             (i+1,rts@[(t,epv_i)]))
                          (0,[]) rs in
...
and init_prm () =
  ...
  gns.ns_e <- (epf,ref(Ast.Etr_V(RP.R[|RP.A(R.Idx 0);RP.A(R.Idx 1)|],RP.R[|RP.A(R.Idx 0);RP.A(R.Idx 1);RP.A(R.Agl(2,RP.A(R.Idx 3)))|])))::gns.ns_e;
  let se_chr =
    "\tNS_E_DYN_"^(Sgn.print epf)^":\n"^
    (*"\t\tdb 0,0b1,0,0b1,0b10000000,0,0,0b1\n"^ *)
    "\t\tdq 0b00000000_00000001_00000000_00000001_10000000_00000000_00000000_00000001\n"^
  ...
  gns.ns_e <- (Ast.Axm._none,ref(Ctr 1))::gns.ns_e;
  ...
  (se_chr^se_dgt^se_u_al^se_l_al^se_emt^se_pp_v,em_chr^em_dgt^em_l_al^em_u_al^em_emt^em_pp_v,ns,gns)
and emt_exe m =
      ...
      (fun (i,n,e0,e1) b ->
         if b then
           if i<9 then
             let e0 =
               e0^
               "\tmov QWORD [rsp+8*"^(string_of_int n)^"],"^(emt_reg_x86 i)^"\n" in
             let e1 =
               "\tmov "^(emt_reg_x86 i)^",QWORD [rsp+8*"^(string_of_int n)^"]\n"^
               e1 in
             (i+1,n+1,e0,e1)
           else
             let e0 =
               ...
               "\tmov QWORD [rsp+8*"^(string_of_int n)^"],rdi\n" in
             let e1 =
               "\tmov rdi,QWORD [rsp+8*"^(string_of_int n)^"]\n"^
...
and subst_ptn_atm lt (a0:R.r_atm) =
  ( match a0 with
    ...
    | R.Agl(ia,pa) -> R.Agl(subst lt ia,subst_ptn lt pa)
            ...
            Array.fold_left
              (fun (i,e1) ri ->
                 ...
                 let e1 =
                   ...
                   e1_0^
                   "\tpop rdi\n"^
                   "\tmov QWORD [rdi+8*1+8*"^(string_of_int i)^"],"^(emt_reg_x86 i1)^"\n"^
...
and emt_ir i1 gns ns iv p =
  ...
  ( match p with
    ...
    (*let i = idx s r in
      ...
      "\tbt r12,"^(string_of_int i)^"\n"^
      "\tjmp err\n" *)
    | Mtc(_,_) -> err "emt_ir Mtc 0"
    (* let s1 = Array.copy s0 in
       let i0 = csm_ptn_iv r s1 in
       let c0 = cmt ("\t? "^(emt_ptn i0)) in
      ...
      let s0 =
        ( match n with
          ...
          | IR_Glb_Call(n,x,y) ->
            ...
            let v0 = newvar () in
            let v1 = newvar () in
            let _ = unify [] (Imp(Var v0,Var v1)) (Var yf) in
            ( match af with
              ...
              | af ->
                ( match af with
                  | Ctr j ->
                    ...
                    let ix = csm_ptn_iv (mk_idx_ptn x) iv in
                    let icx = RP.A(R.Agl(im,ix)) in
                    let _ = mk_idx_iv !ns iv icx (mk_idx_ptn y) in
                    let e1 =
                      "\tmov "^(emt_reg_x86 im)^","^(string_of_int j)^
                      "\tbts r12,"^(string_of_int im)^"\n"^
                      "\tret\n" in
                  ...
                  | Etr_V(f_i0,f_i1) ->
                    ...
                    let iy = alc_idx (rset_iv iv) v1 in
                    let s0 = RSet.ini () in
                    let _ = rset_ptn s0 ix in
                    let e1 =
                      e_0^
                      (emt_mov_ptn_to_ptn s0 ix f_i0)^
                      "\tcall NS_E_"^(Sgn.print ep)^"\n"^
                      (emt_mov_ptn_to_ptn (rset_iv iv) f_i1 iy)^
                      e_1
                    in
                    e1
                  (*  let sf = Hashtbl.create 10 in
                      ...
                      let c0 = cmt ("\t"^(pnt_name n)^" "^cx^" ⊢ "^cy^rtl^(pnt_ptn y)) in
                      let l_0 = "call_c_"^(lb ()) in
                      let l_1 = "call_nc_"^(lb ()) in
                      c0^e1^em^
                      ex0^
          ...
          (*let bs = Bytes.of_string c in
            ...
            "\tand r12,~"^(emt_0b ir)^"\n" in
            c0^e0 *)
          | IR_Exp(Ast.ExpCst(Cst.R64 _),_,Rcd_Ptn.A _) -> err "emt_ir T1"
          (*let ir = idx_crt s r in
            let c0 = cmt ("\t» "^(print_exp (Ast.Atm(Ast.R64 x)))^" |~ "^(emt_ptn (Rcd_Ptn.A ir))^rtl^(pnt_ptn (Rcd_Ptn.A r))) in
