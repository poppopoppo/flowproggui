...
module Grm = struct
  ...
  and etr_cnc =
    | Cnc_End of cn * rle_flg * rle
    | Cnc_Seq of cn * rle_flg * rle * etr_cnc list
  and cn =
    | CN of string
    | CN_A
  and 'a etr_act =
    | Act_End of 'a * rle_flg * rle
    | Act_Seq of 'a * rle_flg * rle * ('a etr_act) list
  and rle = ptn list
  and ptn =
    | Lst of atm
    | Opn of atm
  ...
  and print_etr_cnc =
    ( function
      | Cnc_End(rn,f,r) ->
        ( match rn with
          | CN rn ->
            "\t"^(print_rle_flg f)^rn^" : "^(print_rle r)^"\n"
          | CN_A ->
            "\t"^(print_rle r)^"\n" )
      | Cnc_Seq(rn,f,r,g) ->
        ( match rn with
...
and emt_ptn_grm i ns ep f r j =
  ( match r with
          ...
          | Grm.Lst a ->
            ...
            ( match a with
              | Grm.Txt s ->
                ...
                "\tmov QWORD [rsp+16*"^(string_of_int j)^"],0\n"^
                "\tmov QWORD [rsp+8*1+16*"^(string_of_int j)^"],rsi\n"
              | Grm.Name f ->
                let mf = get_ns_m_t !ns f in
                let m_ns = get_ns_m !ns f in
                let lb_nc = lb () in
    ...
    | [] -> "" )
and emt_m gns (ns:ns_v ref) ld el =
    ...
    ( match p0 with
      ...
      | RP.R rs ->
        let e0 =
          "\tmov rax,rbx\n"^
          "\tmov rbx,QWORD [rbx]\n"^
          "\tmov rdi,0x0000_"^(Printf.sprintf "%04x" (Array.length rs))^"_0000_ffff\n"^
          "\tmov QWORD [rax],rdi\n"^
          "\tmov "^(emt_reg_x86 i1)^",rax\n"^
          "\tbtr r12,"^(string_of_int i1)^"\n" in
        s0.(i1)<-true;
