      ...
      let (e1,_) =
        Array.fold_left
          (fun (e1,i) r ->
             let ei0 =
               "\tpush "^dst^"\n" in
             let ei1 = emt_get_ptn tbv idx r "rax" in
             let ei2 =
               "\tmov rdx,rax\n"^
               "\tpop rdi\n"^
...
and emt_ptn_set_ptn tbv idx0 idx1 r0 r1 =
    ...
    | R rs0,R rs1 ->
      let (_,e0) =
        Array.fold_left
          (fun (i,e0) ri ->
             let e1 =
               emt_ptn_set_ptn tbv idx0 idx1 ri rs1.(i) in
             (i+1,e0^e1) )
          (0,"") rs0 in
      (cmt c0)^e0
    | _,_ -> "; emt_ptn_set_ptn\n" )

and emt_dec_ptn idx r =
...
and emt_ir ih s p =
  ...
  ( match p with
    | Ret r ->
      ...
      let e0 =
        (clear s)^
        (emt_get_ptn "r12" emt_reg i0 "rax") in
      let e2 =
        "\tret\n" in
      c0^e0^e2
    | Agl(r,ps) ->
      ...
      let l_0 = lb () in
      let e0 =
        (emt_get_ptn "r12" emt_reg i0 "rax")^
        "\tcmp rax,0\n"^
        "\tmov rdi,rax\n"^
        "\tje "^l_0^"\n"^
        "\tmov rdi,0\n"^
    ...
    | IL_Glb_Call(n,r) ->
      ...
      let er = emt_get_ptn "r12" emt_reg ir "rax" in
      let e0 =
        ( match n with
          | _ -> "emt_ir_glb_call\n" ) in
      er^e0
          ...
          | IR_Id(r,rs) ->
            ...
            let (_,e0) =
              Array.fold_left
                ( fun (i,e0) ri ->
                    let e1 =
                      emt_ptn_set_ptn "r12" emt_reg emt_reg ir ri in
                    (i+1,e0^e1) )
                (0,"") irs in
            let e1 =
              emt_dec_ptn emt_reg ir in
            c0^e0^e1
          | IR_Call((_,_),_) ->
            "; ir_call\n"
          | IR_Glb_Call(n,x,y) ->
            ( match n with
              | Tkn.Etr_N "add" ->
                ...
                let e0 =
                  (emt_ptn_set_ptn "r12" emt_reg emt_reg ix p)^
                  (emt_dec_ptn emt_reg ix)^
                  "\tmov r9,"^(emt_reg i0)^"\n"^
                  "\tmov r10,"^(emt_reg i1)^"\n"^
                  "\tadd r9,r10\n"^
                  "\tmov rdi,r9\n"^
              ...
              | Tkn.Etr_N "cmp" ->
                ...
                let e0 =
                  ...
                  "\tor r12,"^(emt_0b i3)^"\n"^
                  (emt_ptn_set_ptn "r12" emt_reg emt_reg py iy) in
                c0^e0
              | Tkn.Etr_N f ->
                ...
                let e0 =
                  "\nmov rbx,0\n"^
                  "\nnot rbx\n"^
                  (emt_ptn_set_ptn "rbx" emt_reg emt_reg_tmp ix i0)^
                  (emt_dec_ptn emt_reg ix) in
