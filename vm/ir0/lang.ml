...
and emt_get_ptn s tbv idx r dst =
  ...
  ( match r with
    ...
    | R rs ->
      ...
      (cmt c0)^e1^"\tclc\n"
    | _ -> "; emt_get_ptn\n" )
and emt_set_ptn s tbv idx src r =
        ...
        Array.fold_left
          (fun (e0,i) r ->
             let ei =
               ...
               "\tpush rbx\n"^
               "\tmov rbx,["^src^"]\n"^
               "\tbt rbx,"^(string_of_int i)^"\n"^
               "\tmov rbx,["^src^"+8*"^(string_of_int (i+1))^"]\n" in
             let e1 = emt_set_ptn s tbv idx "rbx" r in
             let e2 =
               "\tpop rbx\n"^
               "\tpop "^src^"\n" in
             (e0^ei^e1^e2,i+1))
...
and emt_ptn_set_ptn s tbv idx0 idx1 r0 r1 =
    ...
    | R rs0,R rs1 ->
      let (_,e0) =
        Array.fold_left
          (fun (i,e0) ri ->
             let e1 =
               emt_ptn_set_ptn s tbv idx0 idx1 ri rs1.(i) in
             (i+1,e0^e1) )
          (0,"") rs0 in
      (cmt c0)^e0
    | _,_ -> "; emt_ptn_set_ptn\n" )
...
and emt_ir ih s p =
  ...
  ( match p with
    ...
    | Agl(r,ps) ->
      ...
      let e0 =
        "\tpush r14\n"^
        (emt_get_ptn s "r12" emt_reg_x86 i0 "r14")^
        "\tcmp r14,0\n"^
        "\tpush rbx\n"^
        "\tmov rbx,r14\n"^
        "\tje "^l_0^"\n"^
      ...
      let (_,a0) =
        Array.fold_left
          (fun (i,a) (r,p) ->
             ...
             let (ep,w0) = push_reg s x86_reg_lst in
             ...
             let ei =
               ...
               "\tjc "^lc0^"\n"^
               "\tpush rbx\n"^
               (emt_set_ptn s "r12" emt_reg_x86 "rbx" ii)^
               "\tpop rbx\n"^
               ep^
               "\tcall dec_r\n"^
               (pop_reg w0)^
    ...
    | IL_Glb_Call(n,r) ->
      ...
      let er = emt_get_ptn s "r12" emt_reg_x86 ir "rax" in
      let e0 =
        ( match n with
          | _ -> "emt_ir_glb_call\n" ) in
      "\tpush rax\n"^
          ...
          | IR_Id(r,rs) ->
            ...
            let (_,e0) =
              Array.fold_left
                ( fun (i,e0) ri ->
                    let e1 =
                      emt_ptn_set_ptn s "r12" emt_reg_x86 emt_reg_x86 ir ri in
                    (i+1,e0^e1) )
                (0,"") irs in
            let e1 =
          ...
          | IR_Glb_Call(n,x,y) ->
            ( match n with
              | Tkn.Etr_N "add" ->
                ...
                let e0 =
                  (emt_ptn_set_ptn s "r12" emt_reg_x86 emt_reg_x86 ix p)^
              ...
              | Tkn.Etr_N "cmp" ->
                ...
                let e0 =
                  ...
                  "\tor r12,"^(emt_0b i3)^"\n"^
                  (emt_ptn_set_ptn s "r12" emt_reg_x86 emt_reg_x86 py iy) in
                c0^e0
              | Tkn.Etr_N f ->
                ...
                let e0 =
                  "\nmov r14,0\n"^
                  "\nnot r14\n"^
                  (emt_ptn_set_ptn s "r14" emt_reg_x86 emt_reg_tmp ix i0)^
                  (emt_dec_ptn s emt_reg_x86 ix) in
                let (e1,sl) = push_s emt_reg_x86 s in
                let iy = idx_crt_ptn s y in
                let c0 = cmt ("\t"^(Tkn.print_etr n)^" "^(emt_ptn ix)^" ⊢ "^(emt_ptn iy)^rtl^(print_ty y)) in
                let lc0 = lb () in
                let lc1 = lb () in
                let (ep,w) = push_reg s x86_reg_lst in
