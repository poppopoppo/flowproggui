...
let main () =
  ...
  let shell_view =
    ...
    insert (Lang.Tkn.print (let ((_,k),_,_) = !st in k));
    insert_arr ();
    ...
    let key_press k =
      ...
      then
        ...
        st:= Implib.evo !st (Implib.ast_from_string line);
        insert ("~~~~~~~~~~~~~~~~~~~\n"^
                (Implib.print !st));
        insert_arr ();
      ...
      ~callback:(fun s ->
          (match s with
           ...
           | MODIFIED s ->
              ...
              let s = Lang.print (Lang.mk_ir_mdl (snd mdl)) in
              (* Util.pnt true s; *)
