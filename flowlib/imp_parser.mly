...
%token L_PRN R_PRN  CO_PRD_STT  APP R_APP DOT D_EXP COPRD_END PRD_END
%token TEST ACT SPL END_FLW_LST FOR_ALL MDL CLS_NAM MDL_END L_BLK R_BLK  COPRD SEQ
%token IO END_GL_ETR_LST CLS PRJ  N  SLH CMT L_HLZ R_HLZ M_HLZ R_HLZ_TAIL COPRD_TAIL PRD_TAIL
(*
...
glb_etr:
  | LCE NAM typ_def DEF vh_frm_code {
    let (src,dst) = $3 in
  ...
  | vh_frm_top tail_code
    {
      ...
      | `None -> $1
      | `Some t -> Imp.Seq ($1,t)
      | `CoPrd (x,y,_) -> Imp.Seq ($1,Code_CoPrd { pre=x; post=y })
    }
  ;
tail_code:
  | ARR_END { `None }
  ...
  ;
coprd:
  | CO_PRD_STT vh_frm_top coprd_list COPRD_END vh_frm_code coprd_tail
    {  `CoPrd ($2,$3,$4) }
  ;
coprd_list:
  | { [] }
  ...
  ;
coprd_tail:
  | COPRD_TAIL ARR vh_frm_code { }
  | COPRD_TAIL prd {}
  | COPRD_TAIL coprd {}
  | COPRD_TAIL h_frm {}
  ;
prd:
  | PRD_STT vh_frm_top prd_list PRD_END vh_frm_code prd_tail
    {  `CoPrd ($2,$3,$4) }
  ;
prd_list:
  |  {}
