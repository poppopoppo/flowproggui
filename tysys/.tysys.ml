...
let fresh e i =
  ...
  let rec fresh_n e =
    ( match e with
      | Val j -> if i=j then Val n else e
...
let cmp_subst sl : cxt =
  ...
  let rec f c0 l =
    ...
    ( match l with
      ...
      | hd::tl ->
        Hashtbl.iter
          (fun i e -> Hashtbl.add c0 i (subst hd e))
          c0;
        Hashtbl.iter
          (fun i e ->
             ( try
...
let root = Sgn.ini ()
let arg = Sgn.ini ()
let typing (g:gma) c : (cxt * tm) =
  ( match c with
    | Exp e1 -> typing g e1
    | Rtn ->
      let a = Sgn.ini () in
      (cxt_ini (),Tm[Prm imp;Val a;Val a])
    | Seq (c0,c1) ->
...
let typing_exp g e =
  ( match e with
    ...
    | Opr_Rcd r -> raise Fail
    | Root -> raise Fail
    | Arg -> raise Fail
    | App (e1,e2) -> raise Fail
    | Prj (e1,i) -> raise Fail 
  )
