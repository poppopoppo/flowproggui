...
let (<*^) s g = subst_agl_gma s g
let unify_agl_gma g1 g2 =
  pnt true ("enter unify_agl_gma:\n"^
            (print_scm_hd (agl_gma_to g1))^","^
            (print_scm_hd @@ agl_gma_to g2)^"\n");
  let gm = SgnMap.merge
      (fun _ v1 v2 ->
         ( match v1,v2 with
           | None,_ -> None
           ...
           | Some v1,Some v2 ->
             Some (v1,v2)))
      g1 g2 in
  let w = SgnSet.union (set_of_map g1) (set_of_map g2) in
  let r = unify w (SgnMap.fold (fun _ v r -> v::r) gm []) in
  pnt true ("return unify_agl_gma:\n"^
            (print_scm_hd @@ agl_gma_to r)^"\n");
  r
let unify_typ_gma g1 g2 =
  ...
  r
let mrg_gma (ga0:agl_gma) (ga1:agl_gma)  : (agl_gma * cxt) =
  let b0 = unify_agl_gma ga0 ga1 in
  let ga2 =
    SgnMap.merge
      (fun _ a0 a1 ->
         ( match a0,a1 with
           | None,Some a1 -> Some (b0<*a1)
           | Some a0,_ -> Some (b0<*a0)
           | _ -> None
         ))
      ga0 ga1 in
  (ga2,b0)
...
let rec typing_vh g (tg:typ_env) (gv:mdl_gma) c s0 d0 : (cxt * typ_gma) =
  ...
  pnt true ("return typing_vh"^(Sgn.print lb0)^":"^
            ...
            "\n");
  q
and typing_nd g tg gv (e:nd) r d : (cxt * typ_gma)=
  let lb0 = sgn() in
  ...
  let q =
    ( match e with
      ...
      | Cho i1 ->
        ...
        (unify SgnSet.empty [(d,((Prm fld)<+(Val v1))-*(Val v0))],ga)
      ...
      (fun g e ->
         ( match e with
           | Etr (_,(f,_,_,c)) ->
             ...
             let (b0,ga) = typing_vh g typ_env StgMap.empty
                 (vh_of_code c) src dst in
             let (ga1,b1) =
               SgnMap.fold
                 (fun _ (y1,y2) (r,b) ->
                    ( match y1,y2 with
                      | Val k2,_ -> (SgnMap.add k2 y2 r,b)
                      | _ ->
                        ( try
                            let bt =
                              BatList.find_map
                 ...
                 ga (SgnMap.empty,cxt_ini()) in
             let r = Etr(SgnMap.map (fun v -> Some v) ga1,
                         (f,(b0*~b1)<*src,(b0*~b1)<*dst,c)) in
             g@[r]
           | Etr_Clq (_,l) ->
             ...
             let (ga1,_) =
               SgnMap.fold
                 ...
                 ga (SgnMap.empty,SgnMap.empty) in
             let r = Etr_Clq
